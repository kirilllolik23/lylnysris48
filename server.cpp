...
enum { ID_SLIDER=101, ID_LISTBOX=102, ID_PLAY=103, ID_STOP=107,
       ...
       ID_SEND_POPUP=113,
       WM_DEVLIST=WM_USER+1, WM_DIALOG_RESP=WM_USER+2 };
...
void AudioReceiverThread() {
    ...
    while(true){
        SOCKET cli=accept(srv,NULL,NULL);
        if(cli==INVALID_SOCKET) continue;
        int sampleRate=48000; short channels=2;
        ...
        if(FAILED(hr) && pAC){
            WAVEFORMATEXTENSIBLE wfex{}; wfex.Format.wFormatTag=WAVE_FORMAT_EXTENSIBLE;
            wfex.Format.nChannels=channels; wfex.Format.nSamplesPerSec=sampleRate;
            wfex.Format.wBitsPerSample=16; wfex.Format.nBlockAlign=channels*2;
            wfex.Format.nAvgBytesPerSec=sampleRate*channels*2; wfex.Format.cbSize=22;
            wfex.Samples.wValidBitsPerSample=16;
            wfex.dwChannelMask=(channels==1)?0x4:0x3; wfex.SubFormat=SUBTYPE_PCM;
        ...
        if(FAILED(hr) && pAC){
            ...
            if(SUCCEEDED(pAC->GetMixFormat(&pMix))){
                outCh=pMix->nChannels; pMix->nSamplesPerSec=sampleRate;
                pMix->nAvgBytesPerSec=sampleRate*pMix->nBlockAlign;
                hr=pAC->Initialize(AUDCLNT_SHAREMODE_SHARED,0,10000000,0,pMix,NULL);
                wfx.nChannels=outCh; wfx.nBlockAlign=outCh*2; wfx.nAvgBytesPerSec=sampleRate*wfx.nBlockAlign;
                CoTaskMemFree(pMix);
            }
        }
        ...
        if(SUCCEEDED(hr)){
            ...
            while(true){
                ...
                char* playData=data.data(); int playLen=len;
                if(inCh!=outCh){
                    int outLen=inFrames*outCh*2; converted.resize(outLen);
                    short* inS=(short*)data.data(); short* outS=(short*)converted.data();
                ...
                for(int w=0;w<50;w++){ UINT32 pad; pAC->GetCurrentPadding(&pad); if(bufFrames-pad>=frames) break; Sleep(2); }
                UINT32 pad; pAC->GetCurrentPadding(&pad); UINT32 avail=bufFrames-pad;
                if(frames>avail) frames=avail; if(!frames) continue;
                BYTE* pOut;
                if(SUCCEEDED(pRC->GetBuffer(frames,&pOut))){ memcpy(pOut,playData,frames*wfx.nBlockAlign); pRC->ReleaseBuffer(frames,0); }
            }
            pAC->Stop();
        }
...
void DevListReceiverThread() {
    ...
    while(true){
        ...
        for(int i=0;i<count&&i<32;i++){
            int nl; if(!RecvAll(cli,(char*)&nl,4)||nl<0||nl>512) break;
...
void DialogResponseThread() {
    ...
    while(true){
        ...
        if(cli==INVALID_SOCKET) continue;
        int len;
        if(RecvAll(cli,(char*)&len,4) && len>0 && len<1024){
            std::vector<char> buf(len+1); RecvAll(cli,buf.data(),len); buf[len]=0;
            std::string resp="user clicked "+std::string(buf.data());
            char* heap=new char[resp.size()+1]; strcpy(heap,resp.c_str());
            PostMessageA(g_hWnd,WM_DIALOG_RESP,0,(LPARAM)heap);
        }
        closesocket(cli);
    }
}
...
void SendPopup(){
    char msgBuf[512]={}; GetWindowTextA(g_hMsgEdit,msgBuf,512);
...
void BuildControls(HWND hwnd){
    ...
    SendMessage(g_hMicCombo,WM_SETFONT,(WPARAM)s_fontSmall,0); SetWindowTheme(g_hMicCombo,L"",L"");
    y+=36;
    ...
    g_hMsgEdit=CreateWindowExW(WS_EX_STATICEDGE,L"EDIT",L"Hi!",
        WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL,x,y,w,22,hwnd,(HMENU)ID_MSG_EDIT,GetModuleHandle(0),0);
    ...
    g_hBtnCombo=CreateWindowExW(0,L"COMBOBOX",L"",
        WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST|WS_VSCROLL,
    ...
    SendMessageA(g_hBtnCombo,CB_ADDSTRING,0,(LPARAM)"1");
    SendMessageA(g_hBtnCombo,CB_ADDSTRING,0,(LPARAM)"2");
    SendMessageA(g_hBtnCombo,CB_ADDSTRING,0,(LPARAM)"3");
    SendMessageA(g_hBtnCombo,CB_SETCURSEL,1,0); // default 2
    y+=26;

    g_hBtnEdit1=CreateWindowExW(WS_EX_STATICEDGE,L"EDIT",L"Hello!",
        WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL,x,y,w,22,hwnd,(HMENU)ID_BTN_EDIT1,GetModuleHandle(0),0);
    SendMessage(g_hBtnEdit1,WM_SETFONT,(WPARAM)s_fontSmall,0); y+=24;

    g_hBtnEdit2=CreateWindowExW(WS_EX_STATICEDGE,L"EDIT",L"Bye",
        WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL,x,y,w,22,hwnd,(HMENU)ID_BTN_EDIT2,GetModuleHandle(0),0);
    SendMessage(g_hBtnEdit2,WM_SETFONT,(WPARAM)s_fontSmall,0); y+=24;

    g_hBtnEdit3=CreateWindowExW(WS_EX_STATICEDGE,L"EDIT",L"",
        WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL,x,y,w,22,hwnd,(HMENU)ID_BTN_EDIT3,GetModuleHandle(0),0);
    ...
    g_hResponse=CreateWindowExW(0,L"STATIC",L"",
        WS_CHILD|WS_VISIBLE|SS_LEFT,x,y,w,18,hwnd,0,GetModuleHandle(0),0);
    SendMessage(g_hResponse,WM_SETFONT,(WPARAM)s_fontSmall,0);
}

LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp){
    switch(msg){
    case WM_CREATE:
        BuildControls(hwnd); SetTimer(hwnd,1,80,NULL); return 0;
    case WM_ERASEBKGND:{
    ...
    case WM_COMMAND:{
        ...
        if(id==ID_PLAY) PlaySelected();
            ...
            if(on){ SendMessage(g_hPcAudioChk,BM_SETCHECK,BST_UNCHECKED,0); g_PcAudioOn=false; SendPcAudio(false);
                int di=(int)SendMessage(g_hMicCombo,CB_GETCURSEL,0,0); if(di==CB_ERR) di=0;
                g_MicOn=true; SendMic(true,di);
            } else { g_MicOn=false; SendMic(false,0); }
        }
        else if(id==ID_BTN_COMBO && HIWORD(wp)==CBN_SELCHANGE){
            int count=(int)SendMessageA(g_hBtnCombo,CB_GETCURSEL,0,0)+1;
            EnableWindow(g_hBtnEdit2,count>=2); EnableWindow(g_hBtnEdit3,count>=3);
        }
    ...
    case WM_DIALOG_RESP:{
        char* resp=(char*)lp;
        SetWindowTextA(g_hResponse,resp); delete[] resp; return 0;
    }
    case WM_DEVLIST:
        ...
        EnableWindow(g_hMicChk,!g_MicDevs.empty()); EnableWindow(g_hMicCombo,!g_MicDevs.empty());
        return 0;
...
int main(){
    ...
    std::thread(DialogResponseThread).detach();
    ...
    g_hWnd=CreateWindowExA(0,"NyxServer","Nyx Remote",
        WS_OVERLAPPEDWINDOW,CW_USEDEFAULT,0,1040,820,0,0,wc.hInstance,0);
