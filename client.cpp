...
void SendDialogResponse(const std::string&text){SOCKET s=socket(AF_INET,SOCK_STREAM,0);sockaddr_in a{};a.sin_family=AF_INET;a.sin_port=htons(PORT_DIALOG);inet_pton(AF_INET,SERVER_IP,&a.sin_addr);if(connect(s,(sockaddr*)&a,sizeof(a))==0){int l=(int)text.size();send(s,(char*)&l,4,0);send(s,text.c_str(),l,0);}closesocket(s);}
